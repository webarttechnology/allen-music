	...
	function init_srmp3_importTemplates(){
		...
		$('.srmp3_import_overlay').click(function(e){
			...
			var data = {
				...
				nonce: sonaar_admin_ajax.ajax_nonce,
				filename: json_file
			};
			$.post(
				
				sonaar_admin_ajax.ajax_url, 
				data, 
				function(response) {
					var obj;
		...
		if($('.srmp3-generate-bt').length && $('#acf_albums_infos .ffmpeg-not-installed').length === 0){
			...
			function handleInputChange() {
				...
				if ($('#saveChangeMessage').length === 0) {
					...
					//$('.srmp3-post-all-audiopreview-bt').after('<span id="saveChangeMessage" style="margin-left:10px; color:red;">Save changes and refresh this page before generate previews.</span>');
				}
			}
		}
	}

	function init_srmp3_indexTracks(){
		...
		function indexPosts_AJAX(offset, originalText) {
			$.ajax({
				url: sonaar_music_pro.ajax_url,
				type: 'post',
				dataType: 'json',
				data: {
					action: 'index_alb_tracklist_for_lazyload',
					nonce: sonaar_music_pro.ajax_nonce,
					offset: offset
				},
