...
class Sonaar_Music_Widget extends WP_Widget{
    ...
    */
    public function widget ( $args, $instance ){
            ...
            $lazy_load = ( isset( $this->shortcodeParams['lazy_load'] ) && $this->shortcodeParams['lazy_load'] === 'true' )? true : false;
        ...
        foreach( $playlist['tracks'] as $key1 => $track){
            ...
            $track_artwork_container = ( $this->getOptionValue('track_artwork_play_button') ) ? '<div class="sr_track_cover"><div class="srp_play"><i class="sricon-play"></i></div><img src=' . esc_url( $track_image_url ) . ' /></div>' : '<img src=' . esc_url( $track_image_url ) . ' class="sr_track_cover" />' ;
            ...
            */  
            if(function_exists( 'run_sonaar_music_pro' ) &&  get_site_option('SRMP3_ecommerce') == '1' ){
                ...
                $cf_data_formatted .= $this->getTermsForFilters($postid, 'playlist-tag');
            ...
            */
            ...
            $format_playlist .= ( !( isset($this->shortcodeParams['hide_cf_data']) && $this->shortcodeParams['hide_cf_data'] == 'true') )? $cf_data_formatted : '';
        ...
        $widgetPart_tracklist = ($playerWidgetTemplate == 'skin_boxed_tracklist' && $trackNumber <= 1 && isset($this->shortcodeParams['one_track_boxed_hide_tracklist']) && $this->shortcodeParams['one_track_boxed_hide_tracklist'] == "true") ? '<div class="playlist" id="playlist_'. $widget_id .'" style="display:none;">' : '<div class="playlist" id="playlist_'. $widget_id .'">';
...
<?php endif ?>
  ...
  <?php
   ...
   // $start_time = microtime(true);
        ...
        if(!$category){
            //$order = (isset($this->shortcodeParams['order']) && !empty($this->shortcodeParams['order'])) ? $this->shortcodeParams['order'] :  'DESC';   // default order
            //$orderby = (isset($this->shortcodeParams['orderby']) && !empty($this->shortcodeParams['orderby'])) ? $this->shortcodeParams['orderby'] : 'date';  // use the entire $order_raw as orderby
            $ordering['order'] = (isset($this->shortcodeParams['order']) && !empty($this->shortcodeParams['order'])) ? $this->shortcodeParams['order'] :  'DESC';   // default order
            $ordering['orderby'] = (isset($this->shortcodeParams['orderby']) && !empty($this->shortcodeParams['orderby'])) ? $this->shortcodeParams['orderby'] : 'date';  // use the entire $order_raw as orderby
